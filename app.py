...
# Initialize session state for tracking full result dicts across tabs
    ...
    if video_file is not None:
        ...
        if st.button("Run Video Analysis"):
            ...
            with st.spinner("Processing video frames, extracting audio, and running AI reasoning..."):
                ...
                st.info(f"**Phi-3 Mini Video Verdict:**\n\n{vid_res.get('phi_analysis', 'No AI analysis available.')}")
    ...
    if image_file is not None:
        ...
        if st.button("Run Image Analysis"):
            ...
            with st.spinner("Analyzing image and extracting metadata..."):
                ...
                # Save full evidence to session state (not just score)
    ...
    if audio_file is not None:
        ...
        if st.button("Run Audio Analysis"):
            ...
            with st.spinner("Extracting acoustic features..."):
                ...
                # Save full evidence to session state
...
if st.button("Generate Final Forensic Report", type="primary"):
    with st.spinner("Running LLM Fusion Engine — reasoning over all evidence..."):
        # Collect full evidence dicts (not just scores)
        all_evidence = {}
        ...
        if not all_evidence:
            ...
            verdict = generate_final_verdict(all_evidence)

            # Display the 3-category breakdown prominently
            ...
            # Now generate narrative from verdict + evidence
            with st.spinner("Generating 3-stage AI investigator report..."):
